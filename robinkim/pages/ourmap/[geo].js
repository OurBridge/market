...
export default function MapPage({ data }) {
    ...
    const 지리정보 = data?.['지리정보'];

    const [clickedData, setClickedData] = useState({
        market: data?.clickedData
    });
    ...
    useEffect(() => {
        ...
        const mapOptions = {
            center: new naver.maps.LatLng(지리정보.latitude, 지리정보.longitude),
            zoom: 16,
            zoomControl: true,
            mapTypeControl: true,
            ...
            <div className="sidebar">
                ...
                <div className="geo_info">
                    {geoCode?.map((item, idx) => {
                        ...
                        return (
                            <a key={idx} href={`/ourmap/${name}?lat=${lat}&lon=${lon}`}>
                                <span>
                                    {item.name}
                                </span>
                            </a>
...
export async function getServerSideProps({ query }) {
    ...
    const clickedData = await getMapData(name);
